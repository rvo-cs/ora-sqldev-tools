create table &&def_pre_ddl_table (
...
comment on table &&def_pre_ddl_table is 'Table for recording DDL statements (trig_ddl_pre)';

comment on column &&def_pre_ddl_table..seq_num        is 'Sequence number (from seq. seq_ddl_pre)';
comment on column &&def_pre_ddl_table..ddl_time       is 'Time when the pre-DDL trigger (trig_ddl_pre) fired';
comment on column &&def_pre_ddl_table..event_type     is 'Type of DDL event';
begin
    &&def_pdb_aware execute immediate q'<comment on column &&def_pre_ddl_table..con_name  is 'Container name'>';
    null;
end;
/
...
comment on column &&def_pre_ddl_table..module         is 'Calling session info: module (if set)';
comment on column &&def_pre_ddl_table..action         is 'Calling session info: action (if set)';
comment on column &&def_pre_ddl_table..client_host    is 'Calling session info! client hostname';
comment on column &&def_pre_ddl_table..client_ip_addr is 'Calling session info! client IP address';
comment on column &&def_pre_ddl_table..client_osuser  is 'Calling session info: client OS user';
comment on column &&def_pre_ddl_table..session_auditing_id is 'Calling session info: session auditing id';
comment on column &&def_pre_ddl_table..ddl_text       is 'DDL statement text';
